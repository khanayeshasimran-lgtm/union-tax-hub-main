...
import { Phone, Copy, Check, Loader2, User, Mail, Hash, TrendingUp, AlertTriangle, RefreshCw } from "lucide-react";
...
export default function CallWorkflow() {
  ...
  const handleSubmit = async () => {
    ...
    if (disposition !== "Not Answered" && !notes.trim()) {
      toast({ title: "Notes required", description: "Add notes for answered calls.", variant: "destructive" }); return;
    }
    setSubmitting(true);
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="kpi-card lg:col-span-1 overflow-hidden p-0">
          ...
          <div className="max-h-[60vh] overflow-y-auto">
            ...
            : leads.length === 0 ? <p className="px-4 py-8 text-center text-sm text-muted-foreground">No leads in queue</p>
            : leads.map((lead) => (
              <button key={lead.id} onClick={() => { setSelectedLead(lead); setDisposition(""); setNotes(""); }}
                className={`w-full text-left px-4 py-3 border-b transition-colors hover:bg-muted/50 ${selectedLead?.id === lead.id ? "bg-primary/5 border-l-2 border-l-primary" : ""}`}>
                <p className="text-sm font-medium text-foreground">{lead.full_name}</p>
                <div className="mt-1 flex items-center gap-2">
                  <StatusBadge status={lead.status} />
                  <span className="text-xs text-muted-foreground">Attempt {lead.attempt_count}/{systemSettings.max_attempt_limit}</span>
                </div>
              </button>
            ))}
          </div>
            ...
            <>
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  <div className="flex items-center gap-2 rounded-lg bg-muted/50 px-3 py-2"><User className="h-4 w-4 text-muted-foreground" /><span className="text-sm">{selectedLead.full_name}</span></div>
                  <div className="flex items-center gap-2 rounded-lg bg-muted/50 px-3 py-2"><Mail className="h-4 w-4 text-muted-foreground" /><span className="text-sm truncate">{selectedLead.email || "No email"}</span></div>
                  <div className="flex items-center gap-2 rounded-lg bg-muted/50 px-3 py-2"><Hash className="h-4 w-4 text-muted-foreground" /><span className="text-sm">Attempt #{selectedLead.attempt_count + 1}</span></div>
                </div>
                {/* Attempt progress bar */}
                <div className="space-y-1">
                  <div className="flex justify-between text-xs text-muted-foreground"><span>Attempt Progress</span><span>{selectedLead.attempt_count}/{systemSettings.max_attempt_limit}</span></div>
                  <div className="h-2 rounded-full bg-muted overflow-hidden">
                    <div className={`h-full rounded-full transition-all ${atLimit ? "bg-destructive" : "bg-primary"}`}
                ...
                <div className="flex items-center gap-3 rounded-xl border-2 border-primary/20 bg-primary/5 p-4">
                  ...
                  <span className="text-2xl font-bold tracking-wide text-foreground">{selectedLead.phone_number}</span>
                  <Button variant="outline" size="sm" onClick={copyPhone} className="ml-auto">
                    {copied ? <Check className="mr-1.5 h-4 w-4" /> : <Copy className="mr-1.5 h-4 w-4" />}{copied ? "Copied" : "Copy"}
                  </Button>
                </div>
                <p className="text-xs text-muted-foreground">Copy the number above and dial in MightyCall. Return here to log the disposition.</p>
              </div>
              <div className="space-y-4 border-t pt-4">
                ...
                {disposition && (
                  <Textarea placeholder={disposition === "Not Answered" ? "Optional notes…" : "Notes (required)…"}
                    value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} />
                )}
                <Button onClick={handleSubmit} disabled={!disposition || submitting} className="w-full">
                  {submitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Submit Disposition
                </Button>
              </div>
            </>
          )}
